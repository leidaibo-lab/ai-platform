...
| --- | --- | --- | --- | --- |
| C1 对话问答 | Web 文本；未来 IM、IDE、API 文本 | 连续、可纠正的自然语言回答 | V0.6 已跑通 Demo | 建立四维基线和可追踪输出 |
...
当前只把 C1 作为主动建设链路。C2-C7 保留已经明确的架构边界和后续接口位置，但在 C1 达到退出条件之前，不横向扩展场景实现。

这一步不是重新建设通用底座，而是通过 C1 证明底座能够准确、实时、稳定且 Token 可控地完成多轮对话，并形成后续六条链路可复用的观测和验收方式。
...
C1 退出条件：完成一组可检索 trace、一份固定版本的真实模型基线报告、一组异常与并发自动化测试，并且每个已知边界都有明确状态。若实现过程中改变 Session/Run API、上下文预算、模型路由、多模态输入或鉴权，再同步 OpenSpec；只补文档、内部观测和评测资产时不扩大稳定能力声明。
...
4. **打通观测**：为每个阶段记录状态、耗时、Token、来源版本和错误分类。
5. **建立基线**：固定模型与策略，运行离线评测和故障测试，记录四维基线，不先拍脑袋设平台统一阈值。
6. **设置门禁**：根据风险设置场景阈值；关键结构化字段、权限和写操作校验必须是确定性硬门禁。
7. **固化契约**：行为、API、鉴权、模型路由、上下文预算或多模态契约变化时同步 OpenSpec，再进入下一条链路。

每条链路至少交付以下证据：一张可检索链路图、一组 fixture、一份基线报告、一组阶段 trace、相关自动化测试，以及明确的已知边界。

## 建议完善顺序

| 顺序 | 链路 | 原因 | 完成标志 |
| --- | --- | --- | --- |
| 1 | C1 对话问答 | 当前已经可运行，最适合先建立四维测量基线 | 固定真实模型 fixture 可重复对比；阶段耗时和分段 Token 可见 |
